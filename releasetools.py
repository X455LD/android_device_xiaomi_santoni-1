...
def ImportMainBootloaderFirmware(info):
  info.script.AppendExtra('ui_print("Flashing Main Bootloader.");')
  ...
  info.script.AppendExtra('package_extract_file("installfirmware-update/adspso.bin", "/dev/block/bootdevice/by-name/dsp");')

def ImportBkpBootloaderFirmware(info):
  info.script.AppendExtra('ui_print("Flashing Backup Bootloader.");')
